...
import { Textarea } from "../../components/input/input-area";
...
const PromptInputForm = forwardRef<HTMLTextAreaElement, PromptInputProps>(
  function PromptInputForm(
    ...
    const handleContainerClick = (e: React.MouseEvent) => {
      // Focus the textarea when clicking on empty space (padding, gaps).
...
/**
 * Header area for context items like reference badges or attachments.
 * Padding is controlled by the parent's size variant via `data-slot`.
 * Hidden automatically in compact variant or when there are no children.
 */
...
const PromptInputTextarea: React.FC<PromptInputTextareaProps> = ({
  ...
  const BASE_STYLES = cn(
    // Reset all ring/border/bg/outline from InputArea's inputVariants so the
...
/**
 * Footer bar for prompt input actions.
 *
 * For radio-group option pickers in the footer (e.g. a model selector),
 * use `DropdownMenu.RadioGroup` + `DropdownMenu.RadioItem` from
 * `@cloudflare/kumo` directly rather than a custom sub-component.
 */
